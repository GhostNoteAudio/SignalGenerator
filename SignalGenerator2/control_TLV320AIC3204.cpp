...
AudioControlTLV320AIC3204::AudioControlTLV320AIC3204()
{
}

void AudioControlTLV320AIC3204::start(uint8_t address)
{
...
void AudioControlTLV320AIC3204::init()
{
    ...
    // 4: Clock Setting Register 1, Multiplexers
    //ret = writeRegister(0x04, 0, 0b00000100); // Low PLL Clock range, BCLK input, MCLK is CODEC_CLKIN (PLL not used to control codec!)
    writeRegister(0x04, 0, 0b00000111); // Low PLL Clock range, BCLK input, PLL is CODEC_CLKIN (PLL IS used to control codec)

    // 5: Clock Setting Register 2, PLL P&R Values
    writeRegister(0x05, 0, 0b11000100); // PLL is powered up, P=4, R=4

    // 6: Clock Setting Register 3, PLL J Values
    writeRegister(0x06, 0, 0b00000100); // J = 4

    // 7: Clock Setting Register 4, PLL D Values (MSB)
    writeRegister(0x07, 0, 0b0); // D = 0

    // 8: Clock Setting Register 5, PLL D Values (LSB)
    writeRegister(0x08, 0, 0b0); // D = 0


    delay(15); // wait for PLL to stabilise
    ...
    writeRegister(0x19, 0, 0b00000011); // CDIV_CLKIN = PLL

    // 26: Clock Setting Register 9, Multiplexers
    writeRegister(0x1A, 0, 0b10000001); // CLKOUT_M divider powered up, M=1
    ...
    // 52: Register 52: GPIO/MFP5 Control Register
    writeRegister(0x34, 0, 0b00010001); // GPIO is CLKOUT, GPIO pin driven to 1
